...
const validateJobApplication = [
  ...
  body('messageMotivation').notEmpty().withMessage('Le message de motivation est requis')
];

// POST /api/jobs - Créer une nouvelle candidature
router.post('/', upload.single('cv'), validateJobApplication, async (req, res) => {
  try {
  ...
  try {
    ...
    if (application.cvPath) {
      ...
      fs.unlink(application.cvPath, (err) => {
        if (err) console.error('Erreur lors de la suppression du fichier CV:', err);
      });
    }
